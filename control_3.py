...
GPIO.setup(limit_1,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
GPIO.setup(limit_2,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)
...
while (1):
    if item == 0:
        while (1):
            if (GPIO.input(button1)==1 and item == 0):
                current_state = 1
                stater()
                state1()
            if (GPIO.input(button2)==1 and item == 1):
                current_state = 2
                stater()
                state2()
            if (GPIO.input(button3)==1 and item == 2):
                current_state = 3
                stater()
                state3()
                current_state = 4
